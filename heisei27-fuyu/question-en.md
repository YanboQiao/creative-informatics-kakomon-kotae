...
\begin{cases}
1, & n < 1, \\
(161 \times f(n-1) + 2457)\ \bmod\ 2^{24}, & \text{otherwise}.
\end{cases}
$$

where $n$ is a non-negative integer and $\bmod$ denotes a modulus operator (the remainder).  
...
\begin{cases}
1, & n < 1, \\
(1103515245 \times g(n-1) + 12345)\ \bmod\ 2^{26}, & \text{otherwise}.
\end{cases}
$$
...
(7) Write a program that computes the smallest positive integer $k$ such that $h(n+k)=h(n)$ for any non-negative integer $n$.  
Write on the answer sheet why the program correctly computes $k$.  
