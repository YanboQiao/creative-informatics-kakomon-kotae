...
def choose_spawn_point(retsu: int) -> int:
...
def update_mokuhyou_ichi(ichi: tuple[int, int], houhou: tuple[int, int]) -> tuple[tuple[int, int], tuple[int, int]]:
    ...
    if y >= gyou - 1:
        spawn_x = choose_spawn_point(retsu)
        return (spawn_x, 0), (1, 1)
...
def draw_board(base: list[list[str]], bullet_ichis: list[tuple[int, int]], mokuhyou_ichi: tuple[int, int], spawn_x: int, gun_position: tuple[int, int]) -> list[list[str]]:
...
def main():
    global retsu, gyou, dx, dy, x, y, score, spawn_x
    retsu, gyou = 9, 15
    dx, dy = 1, 1
    spawn_x = choose_spawn_point(retsu)
    x, y = spawn_x, 0
    ...
    while True:
        ...
        board = draw_board(base, bullet_ichis, mokuhyou_ichi, spawn_x, gun_position)
        print_board(board)
